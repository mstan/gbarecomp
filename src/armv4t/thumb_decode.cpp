...
Instr ThumbDecoder::decode(uint16_t hw, uint32_t pc) {
    ...
    switch (top3) {
        ...
        case 0b111: {
            ...
            //   11101  → BLX (ARMv5+); undefined on ARMv4T
            //   11110  → BL upper half    (fmt 19)
            //   11111  → BL lower half    (fmt 19)
            // → bits(12,11) discriminates: 00, 01, 10, 11.
            uint32_t b12_11 = bits(hw, 12, 11);
            if (b12_11 == 0b00) return fmt18_uncond_branch(hw, pc);
            if (b12_11 == 0b01) return undefined(hw, pc);  // BLX (ARMv5+)
            return fmt19_long_branch(hw, pc);              // 10 or 11
        }
    }
    return undefined(hw, pc);
}
